...
api.interceptors.response.use(
  ...
  (error: AxiosError<{ message?: string }>) => {
    ...
    const message =
      ERROR_MESSAGES[status] || error.response?.data?.message || "Ошибка API";
...
export const getEventRegion = (eventId: string): Promise<string> => {
  ...
  const regionPromise = (async () => {
    try {
      ...
      const participantWithRegion = participants.find(
        (p: { region?: string }) => p.region && p.region.trim() !== "",
      );
...
async function enrichEventsWithRegions(
  events: CompetitionEvent[],
): Promise<CompetitionEvent[]> {
  const regions = await Promise.all(
    events.map((event) => getEventRegion(event.event_id)),
  );
...
export const fetchEventsPaginated = async (
  ...
  try {
    ...
    if (filters.weapon && filters.weapon !== "Любое") {
      params.weapon =
        WeaponReverseMap[filters.weapon as keyof typeof WeaponReverseMap];
    }
    ...
    if (
      filters.serie &&
      filters.serie.length > 0 &&
      !filters.serie.includes("Все")
    ) {
    ...
    const { data: allEvents } = await api.get<CompetitionEvent[]>(
      "/restapi/events",
      { params },
    );
...
export const transformToDisplayEvent = (
...
export const fetchParticipants = (eventId: string) => {
  ...
  return api
    .get(`/restapi/events/${eventId}/participants`)
    .then((r) => r.data)
    .catch(() => []);
};
